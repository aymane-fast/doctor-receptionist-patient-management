...
@section('title', __('patients.edit_patient_title') . ' - ' . $patient->full_name)
        ...
        <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between space-y-4 lg:space-y-0">
            <div class="flex items-center space-x-4">
                ...
                <div>
                    <h1 class="text-3xl font-bold text-gray-900">
                        {{ __('patients.edit_patient_title') }} - <span class="text-gradient">{{ $patient->full_name }}</span>
                    </h1>
                    <p class="text-gray-600 mt-2 text-lg">{{ __('patients.update_patient_info') }}</p>
                </div>
            </div>
            <div class="flex flex-wrap gap-3">
                <a href="{{ route('patients.show', $patient) }}" class="bg-gradient-to-r from-emerald-600 to-emerald-700 hover:from-emerald-700 hover:to-emerald-800 text-white px-6 py-3 rounded-2xl font-medium transition-all duration-200 flex items-center space-x-2">
                    ...
                    <span>{{ __('patients.view_patient') }}</span>
                </a>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div class="space-y-2">
                        ...
                        <select id="gender" 
                            ...
                            <option value="male" {{ old('gender', $patient->gender) == 'male' ? 'selected' : '' }}>{{ __('patients.male') }}</option>
                            <option value="female" {{ old('gender', $patient->gender) == 'female' ? 'selected' : '' }}>{{ __('patients.female') }}</option>
                        </select>
                        @error('gender')
                        <p class="mt-2 text-sm text-red-600 flex items-center space-x-2">
                            <i class="fas fa-exclamation-circle"></i>
                ...
                <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between space-y-4 lg:space-y-0">
                    ...
                    <div>
                        <button type="button" 
                            ...
                            <span>{{ __('patients.delete_patient') }}</span>
                        </button>
                    </div>
                    ...
                    <div class="flex flex-col sm:flex-row items-center space-y-3 sm:space-y-0 sm:space-x-4">
                        ...
                        <button type="submit" class="w-full sm:w-auto bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white px-8 py-3 rounded-2xl font-medium transition-all duration-200 deep-shadow flex items-center justify-center space-x-2">
                            ...
                            <span>{{ __('patients.update_patient') }}</span>
                        </button>
                    </div>
                </div>
            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelector('button[onclick*="delete-form"]').addEventListener('click', function(e) {
        e.preventDefault();
        if (confirm('{{ __('patients.delete_confirmation') }}')) {
